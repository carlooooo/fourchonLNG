...
<body>
  ...
  <?php
  $name = $_GET['name'];
  $sql1 = $db->prepare("SELECT proj_desc_image FROM web_images WHERE webpage_name='$name'");
  $sql1->execute();
  $row = $sql1->fetch( PDO::FETCH_ASSOC );
  if (!empty($row['proj_desc_image'])) {
   ?>
  <div class="col-sm-8">

<?php
  echo'<img src="data:image/jpeg;base64,'.base64_encode( $row['proj_desc_image'] ).'" width="1110" height="340.78"/>';}?>
  </div>
  </div>
                    ...
                    if(substr($imageType,0,5) == "image")
                        {
                          ...
                          $num = $sql1->rowCount();
                          if ($num==0) {
                          $stmt = $db->prepare("INSERT INTO web_images (proj_desc_image, webpage_name) VALUES ('$imageData', '$name')");
                          $stmt->execute();
                          echo "<script>alert('Save successfully')</script>";
                          echo "<meta http-equiv='refresh' content='0'>";
                          }
                          else{
                            $stmt = $db->prepare("UPDATE web_images SET proj_desc_image='$imageData' WHERE webpage_name='$name'");
                            $stmt->execute();
                            echo "<script>alert('Save successfully')</script>";
                            echo "<meta http-equiv='refresh' content='0'>";
                        }
